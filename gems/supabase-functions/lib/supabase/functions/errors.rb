...
module Supabase
  ...
  module Functions
    # Mixin module so that `is_a?(FunctionsError)` works across all error classes.
    module FunctionsError; end

    # Concrete base class for non-HTTP/non-network functions errors.
    class FunctionsBaseError < Supabase::Error
      include FunctionsError
    end

    # Raised when a network or fetch-level error occurs (e.g., connection refused, DNS failure).
    class FunctionsFetchError < Supabase::NetworkError
      include FunctionsError
    end

    # Raised when the relay returns an error (x-relay-error header present).
    class FunctionsRelayError < Supabase::ApiError
      include FunctionsError
    end

    # Raised when the function returns a non-2xx HTTP status code.
    class FunctionsHttpError < Supabase::ApiError
      include FunctionsError
    end
  end
end
